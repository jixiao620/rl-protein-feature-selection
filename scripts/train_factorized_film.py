#!/usr/bin/env python3
"""
Multi-disease training with FiLM-conditioned Factorized DQN.

Key difference from train_factorized.py:
  - FiLMFactorizedQNetwork: disease embedding modulates protein embeddings
    via FiLM (gamma, beta), so different diseases select different proteins
    at inference time.
  - State encoder sees mask+timestep only; disease conditions protein space.
"""
...
def parse_args():
    p = argparse.ArgumentParser("FiLM Factorized DQN: full-feature multi-disease training")
    ...
    p.add_argument("--run-name", type=str, default="film_factorized")
    return p.parse_args()
...
def main():
    ...
    print("=" * 60)
    print("FiLM Factorized DQN — Full Feature Space")
    ...
    for code in args.train_disease_codes:
        ...
        assert X.shape[1] == n_proteins

        disease_data[code] = {
            "X": X,
            "y": y,
            "embedding": load_disease_embedding(args.embedding_path, code),
            "reward_fn": AUROCReward(X=X, y=y, train_size=args.train_size),
        }
        print(f"  {code}: {X.shape[0]} patients, positive rate: {y.mean():.4f}")

    # State: mask(n_proteins) + timestep(1) + disease_emb(10)
    emb_dim = 10
    state_dim = n_proteins + 1 + emb_dim
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\nState dim: {state_dim} (encoder sees {n_proteins+1}, FiLM uses last 10)")
    print(f"Device: {device}")

    q = FiLMFactorizedQNetwork(
        state_dim=state_dim,
        n_proteins=n_proteins,
        protein_emb_dim=args.protein_emb_dim,
        hidden=args.hidden,
        disease_emb_dim=emb_dim,
    ).to(device)
    q_target = FiLMFactorizedQNetwork(
        state_dim=state_dim,
        n_proteins=n_proteins,
        protein_emb_dim=args.protein_emb_dim,
        hidden=args.hidden,
    ...
    run_dir = os.path.join("runs", f"{timestamp}_{args.run_name}_seed{args.seed}")
    ...
    while global_step < args.max_total_steps:
        ...
        if episode % evaluate_freq == 0:
            ...
            print(f"Ep {episode} | step={global_step} | eval_return={eval_ret:.4f} | disease={code}")
